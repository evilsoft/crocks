...
test('Result either', t => {
  ...
  t.throws(fn(null, noop), err, 'throws with null in left')
  t.throws(fn(undefined, noop), err, 'throws with undefined in left')
  t.throws(fn(0, noop), err, 'throws with falsey number in left')
  t.throws(fn(1, noop), err, 'throws with truthy number in left')
  t.throws(fn('', noop), err, 'throws with falsey string in left')
  t.throws(fn('string', noop), err, 'throws with truthy string in left')
  t.throws(fn(false, noop), err, 'throws with false in left')
  t.throws(fn(true, noop), err, 'throws with true in left')
  t.throws(fn({}, noop), err, 'throws with object in left')
  t.throws(fn([], noop), err, 'throws with array in left')

  t.throws(fn(noop, null), err, 'throws with null in right')
  t.throws(fn(noop, undefined), err, 'throws with undefined in right')
  t.throws(fn(noop, 0), err, 'throws with falsey number in right')
  t.throws(fn(noop, 1), err, 'throws with truthy number in right')
  t.throws(fn(noop, ''), err, 'throws with falsey string in right')
  t.throws(fn(noop, 'string'), err, 'throws with truthy string in right')
  t.throws(fn(noop, false), err, 'throws with false in right')
  t.throws(fn(noop, true), err, 'throws with true in right')
  t.throws(fn(noop, {}), err, 'throws with object in right')
  t.throws(fn(noop, []), err, 'throws with array in right')
...
test('Result map properties (Functor)', t => {
  ...
  t.equal(
    Ok(10).map(composeB(f, g)).either(constant(0), identity),
  ...
  t.equal(
    Err(10).map(composeB(f, g)).either(identity, constant(0)),
...
test('Result bimap errors', t => {
  ...
  t.throws(bimap(undefined, noop), err, 'throws with undefined in first argument')
  t.throws(bimap(null, noop), err, 'throws with null in first argument')
  t.throws(bimap(0, noop), err, 'throws with falsey number in first argument')
  t.throws(bimap(1, noop), err, 'throws with truthy number in first argument')
  t.throws(bimap('', noop), err, 'throws with falsey string in first argument')
  t.throws(bimap('string', noop), err, 'throws with truthy string in first argument')
  t.throws(bimap(false, noop), err, 'throws with false in first argument')
  t.throws(bimap(true, noop), err, 'throws with true in first argument')
  t.throws(bimap([], noop), err, 'throws with an array in first argument')
  t.throws(bimap({}, noop), err, 'throws with object in first argument')

  t.throws(bimap(noop, undefined), err, 'throws with undefined in second argument')
  t.throws(bimap(noop, null), err, 'throws with null in second argument')
  t.throws(bimap(noop, 0), err, 'throws with falsey number in second argument')
  t.throws(bimap(noop, 1), err, 'throws with truthy number in second argument')
  t.throws(bimap(noop, ''), err, 'throws with falsey string in second argument')
  t.throws(bimap(noop, 'string'), err, 'throws with truthy string in second argument')
  t.throws(bimap(noop, false), err, 'throws with false in second argument')
  t.throws(bimap(noop, true), err, 'throws with true in second argument')
  t.throws(bimap(noop, []), err, 'throws with an array in second argument')
  t.throws(bimap(noop, {}), err, 'throws with object in second argument')
...
test('Result bimap properties (Bifunctor)', t => {
  ...
  t.equal(
    e(Result.Ok(10).bimap(constant(0), composeB(f, g))),
  ...
  t.equal(
    e(Result.Err(10).bimap(composeB(f, g), constant(0))),
...
test('Result ap errors', t => {
  ...
  const right = bindFunc(x => Result.Ok(noop).ap(x))
...
test('Result chain errors', t => {
  ...
  t.throws(ochain(noop), errNoResult, 'Ok throws with a non Result returning function')
...
test('Result sequence errors', t => {
  ...
  t.throws(oSeqBad(noop), noApplicative, 'Ok without wrapping Applicative throws')

  t.doesNotThrow(eSeqBad(noop), 'allows Err without wrapping Applicative')
...
test('Result traverse errors', t => {
  ...
  t.throws(otrav(undefined, noop), noFunc, 'Ok throws with undefined in first argument')
  t.throws(otrav(null, noop), noFunc, 'Ok throws with null in first argument')
  t.throws(otrav(0, noop), noFunc, 'Ok throws falsey with number in first argument')
  t.throws(otrav(1, noop), noFunc, 'Ok throws truthy with number in first argument')
  t.throws(otrav('', noop), noFunc, 'Ok throws falsey with string in first argument')
  t.throws(otrav('string', noop), noFunc, 'Ok throws with truthy string in first argument')
  t.throws(otrav(false, noop), noFunc, 'Ok throws with false in first argument')
  t.throws(otrav(true, noop), noFunc, 'Ok throws with true in first argument')
  t.throws(otrav([], noop), noFunc, 'Ok throws with an array in first argument')
  t.throws(otrav({}, noop), noFunc, 'Ok throws with an object in first argument')
  ...
  t.throws(etrav(noop, undefined), noFunc, 'Err throws with undefined in second argument')
  t.throws(etrav(noop, null), noFunc, 'Err throws with null in second argument')
  t.throws(etrav(noop, 0), noFunc, 'Err throws falsey with number in second argument')
  t.throws(etrav(noop, 1), noFunc, 'Err throws truthy with number in second argument')
  t.throws(etrav(noop, ''), noFunc, 'Err throws falsey with string in second argument')
  t.throws(etrav(noop, 'string'), noFunc, 'Err throws with truthy string in second argument')
  t.throws(etrav(noop, false), noFunc, 'Err throws with false in second argument')
  t.throws(etrav(noop, true), noFunc, 'Err throws with true in second argument')
  t.throws(etrav(noop, []), noFunc, 'Err throws with an array in second argument')
  t.throws(etrav(noop, {}), noFunc, 'Err throws with an object in second argument')

  const noApplicative = /Result.traverse: Both functions must return an Applicative/
  t.throws(otrav(noop, noop), noApplicative, 'Ok throws when first function does not return an Applicaitve')
  t.throws(etrav(noop, noop), noApplicative, 'Err throws when second function does not return an Applicaitve')
