...
test('or logic function', t => {
  const f = bindFunc(or)

  t.ok(isFunction(or), 'is a function')

  const err = /or: Preds or predicate functions required for first two arguments/
...
test('or with predicate functions', t => {
  const f = x => x < 10
  const g = x => x % 2 === 0
  ...
  t.equals(fn(7), true, 'returns true when first is true and second is false')
...
test('or with Pred in first and function in second', t => {
  ...
  t.equals(fn(7), true, 'returns true when Pred is true and function is false')
...
test('or with function in first and Pred in second', t => {
  const pred = Pred(x => x < 10)
  const f = x => x % 2 === 0
  ...
  t.equals(fn(7), true, 'returns true when function is true and Pred is false')
  t.equals(fn(20), true, 'returns true when function is false and Pred is true')
  t.equals(fn(23), false, 'returns false when function is false and Pred is false')

  t.end()
})

test('or with Preds', t => {
  const f = Pred(x => x < 10)
