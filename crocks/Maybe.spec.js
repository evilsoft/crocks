...
test('Maybe traverse errors', t => {
  ...
  t.doesNotThrow(rtrav(noop, MockCrock), 'Just requires an Applicative returning function in second argument')
  ...
  t.throws(ltrav(noop, noop), TypeError, 'Nothing throws when first function does not return an Applicaitve')

  t.doesNotThrow(ltrav(MockCrock, noop), 'Nothing requires an Applicative returning function in the first arg')
