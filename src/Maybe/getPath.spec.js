...
test('getPath function', t => {
  const fn =
    x => getPath([ 'key' ], x).option('nothing')

  const empty =
    x => getPath([], x).option('nothing')

  t.ok(isFunction(getPath), 'is a function')
...
test('getPath errors', t => {
  const fn = bindFunc(x => getPath(x, {}))

  const err = /getPath: Array of Non-empty Strings or Integers required for first argument/
...
test('getPath object traversal', t => {
  const fn = x =>
    getPath([ 'a', 'b' ], x).option('nothing')

  const empty = x =>
    getPath([], x).option('nothing')
...
test('getPath array traversal', t => {
  const fn = x =>
    getPath([ 0, '1' ], x).option('nothing')
...
test('getPath mixed traversal', t => {
  const fn = x =>
    getPath([ 0, 'a' ], x).option('nothing')
